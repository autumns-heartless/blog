...
### `VipObjectEditToolBar.tsx`

```javascript
import React, { MouseEvent, useContext } from 'react'
import { RGNode, RelationGraphStoreContext } from 'relation-graph-react'
import CircumIcon from './RGDemoComponents/MyDemoIcons'
import { ElNotification } from './RGDemoComponents/MyUIComponents'
import './VipObjectEditToolBar.scss'

const VipObjectEditToolBar: React.FC = () => {
  const graphInstance = useContext(RelationGraphStoreContext)
  const newNodeIdIndex = React.useRef(1)
  const newLineIdIndex = React.useRef(1)
  ...
  const startAddNode = (e: MouseEvent) => {
    graphInstance.startCreatingNodePlot(e, {
      templateText: 'Node',
      templateNode: {
        id: 'node-tmp-xxxxxxxxxxxxx',
        text: '',
        className: 'my-node-template',
      },
      onCreateNode: (x: number, y: number) => {
        console.log('New node:', x, y)
        const newId = newNodeIdIndex.current++
  ...
  const startAddLine = (e: MouseEvent) => {
    ...
    graphInstance.startCreatingLinePlot(e, {
      ...
      onCreateLine: (from: RGNode, to: RGNode) => {
        console.log('New line:', from, to)
        if (to.id) {
          const newLineId = newLineIdIndex.current++
  ...
  return (
    <div
      style={{
        ...
        boxShadow: '0 3px 9px rgba(0,21,41,.08)',
        borderRadius: '10px',
      }}
    >
      <div
      ...
      >
        ...
        >
          <CircumIcon size="20px" name="cloud_on" />
          <div className="c-mb-text">Save</div>
        </div>
        <div
          className="c-mb-button"
          style={{
        ...
        <div
          className={`c-mb-button c-mb-button-c ${
            options.creatingNodePlot ? 'c-mb-button-on' : ''
          }`}
          style={{ width: '100px' }}
          onClick={startAddNode}
        >
          <CircumIcon size="20px" name="circle_plus" />
          <div className="c-mb-text">Node(Click)</div>
        </div>
        <div
          className={`c-mb-button c-mb-button-c ${
            options.creatingNodePlot ? 'c-mb-button-on' : ''
          ...
          style={{ width: '100px' }}
          onMouseDown={startAddNode}
        >
          <CircumIcon size="20px" name="circle_plus" />
          <div className="c-mb-text">Node(Drag)</div>
        </div>
        <div
          className={`c-mb-button c-mb-button-c ${
            options.creatingLinePlot ? 'c-mb-button-on' : ''
          }`}
          style={{ width: '50px' }}
          onClick={startAddLine}
        >
          <CircumIcon size="20px" name="route" />
          <div className="c-mb-text">Line</div>
        </div>
        <div className="c-mb-button" onClick={zoomToFit}>
          <CircumIcon size="20px" name="maximize_2" />
...
export default VipObjectEditToolBar
```

### `VipObjectEditToolBar.scss`
