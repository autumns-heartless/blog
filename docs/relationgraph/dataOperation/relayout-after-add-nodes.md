  ...
  <div>
    <div ref="myPage" class="my-graph" style="height:calc(100vh);">
      <RelationGraph
        ...
        <template #node="{node}">
          <div v-if="node.data && node.data.myType === 'my-add-button'" class="my-add-button">
            <i class="el-icon-circle-plus" @click="addNodesForParent(node)" />
          </div>
...
export default {
  ...
  data() {
    return {
      graphOptions: {
        ...
        defaultNodeColor: 'rgba(128, 128, 255)',
        ...
        layout: {
          ...
          max_per_width: 210, // 根据节点的宽度设置，这个是让图谱看起来偏亮的关键
          min_per_height: 80,
          max_per_height: 80,
        }
        // 这里可以参考"Graph 图谱"中的参数进行设置
      },
      myGraphTreeData: null
    };
  },
  mounted() {
    this.showGraph();
  ...
  methods: {
          ...
          { 'id': 'a', 'text': 'a', children: [
              { 'id': 'b', 'text': 'b', children: [
                  { 'id': 'b1', 'text': 'b1', children: [
                      { 'id': 'b1-add-button', 'text': 'add node', disableDefaultClickEffect: true, data: { myType: 'my-add-button'} },
                    ]
                  },
                  { 'id': 'b2', 'text': 'b2', children: [
                      { 'id': 'b2-1', 'text': 'b2-1', children: [
                          { 'id': 'b2-1-add-button', 'text': 'add node', disableDefaultClickEffect: true, data: { myType: 'my-add-button'} },
                        ]
                      },
                      { 'id': 'b2-2', 'text': 'b2-2', children: [
                          { 'id': 'b2-2-add-button', 'text': 'add node', disableDefaultClickEffect: true, data: { myType: 'my-add-button'} },
                        ]
                      },
                      { 'id': 'b2-add-button', 'text': 'add node', data: { myType: 'my-add-button'} },
                    ]
                  },
                  { 'id': 'b-add-button', 'text': 'add node', disableDefaultClickEffect: true, data: { myType: 'my-add-button'} },
                ]
              },
              { 'id': 'c', 'text': 'c', children: [
                  { 'id': 'c1', 'text': 'c1', children: [
                      { 'id': 'c1-add-button', 'text': 'add node', data: { myType: 'my-add-button'} },
                    ]
                  },
                  { 'id': 'c2', 'text': 'c2', children: [
                      { 'id': 'c2-add-button', 'text': 'add node', disableDefaultClickEffect: true, data: { myType: 'my-add-button'} },
                    ]
                  },
                  { 'id': 'c3', 'text': 'c3', children: [
                      { 'id': 'c3-add-button', 'text': 'add node', disableDefaultClickEffect: true, data: { myType: 'my-add-button'} },
                    ]
                  },
                  { 'id': 'c-add-button', 'text': 'add node', disableDefaultClickEffect: true, data: { myType: 'my-add-button'} },
                ]
              },
              { 'id': 'a-add-button', 'text': 'add node', disableDefaultClickEffect: true, data: { myType: 'my-add-button'} },
            ]
          },
        ],
        'lines': []
      };
      this.myGraphTreeData = __graph_json_data;
      // 将this.myGraphData设置为非响应式，因为他不会与UI绑定
      Object.defineProperty(this, 'myGraphTreeData', {
        configurable: true,
        enumerable: true,
        value: this.myGraphTreeData
      });
      this.initNodeStyle(this.myGraphTreeData.nodes);
      await this.applyMyDataToRelationGraph();
    },
    async applyMyDataToRelationGraph() {
      const graphInstance = this.$refs.graphRef.getInstance();
      await graphInstance.setJsonData(this.myGraphTreeData);
    },
      initNodeStyle(nodes) {
          for (const cNode of nodes) {
              if (cNode.data && cNode.data.myType === 'my-add-button') {
                  cNode.color = 'transparent';
                  cNode.nodeShape = 0;
              }
              if (!cNode.children) continue;
              this.initNodeStyle(cNode.children);
          }
      },
    findNodeInMyTreeData(nodeId, nodes) {
      if (!nodes) {
        return this.findNodeInMyTreeData(nodeId, this.myGraphTreeData.nodes);
      } else {
        for (const cNode of nodes) {
          if (cNode.id === nodeId) {
            return cNode;
          }
          console.log(cNode.id, nodeId);
          if (!cNode.children) continue;
          const result = this.findNodeInMyTreeData(nodeId, cNode.children);
          if (result) {
            return result;
          }
        }
    ...
    async addNodesForParent(buttonNode) {
      const parentNode = buttonNode.lot.parent;
      console.log(' find ', parentNode.id);
      const nodeInTree = this.findNodeInMyTreeData(parentNode.id); // 从原始数据中找到对应的节点
      const randomNewNodesNum = Math.ceil(Math.random() * 3);// 随机生成至多3个新节点
      for (let i = 0; i < randomNewNodesNum; i++) {
        console.log(i, ' of ', randomNewNodesNum);
        const randomId = Math.ceil(Math.random() * 9999);// 随机生成一个id，有概率重复
        ...
        nodeInTree.children.push({
          ...
          children: [
            {
                'id': newNodeId + '-add-button',
                'text': 'add node',
                disableNodeClickEffect: true,
                color: 'transparent',
                nodeShape: 0,
                data: { myType: 'my-add-button'}
            },
          ]
        });
      }
      // 重新排序，把按钮放到最后
      nodeInTree.children.sort((a, b) => (a.data && a.data.myType === 'my-add-button') ? 1 : -1);
      console.log(randomNewNodesNum, nodeInTree.children);
      await this.applyMyDataToRelationGraph();
    }
  }
};
  ...
  <div>
    <div ref="myPage" class="my-graph" style="height:calc(100vh);">
      <RelationGraph ref="graphRef" :options="graphOptions">
        <template #node="{node}">
          <div v-if="node.data && node.data.myType === 'my-add-button'" class="my-add-button">
              <CircumIcon size="20px" name="circle_plus" style="cursor: pointer"  @click="addNodesForParent(node)"/>
          </div>
          <div v-else class="my-node">
            {{ node.text }}
          </div>
...
const myGraphTreeData: RGJsonData = {
    rootId: 'a',
    nodes: [
        { 'id': 'a', 'text': 'a', children: [
            { 'id': 'b', 'text': 'b', children: [
...
const showGraph = async () => {
    await applyMyDataToRelationGraph();
};

const applyMyDataToRelationGraph = async () => {
    const graphInstance = graphRef.value!.getInstance();
    await graphInstance.setJsonData(myGraphTreeData);
};

const findNodeInMyTreeData = (nodeId: string, nodes?: RGNode[]): RGNode | undefined => {
    if (!nodes) {
        return findNodeInMyTreeData(nodeId, myGraphTreeData.nodes);
    } else {
        for (const cNode of nodes) {
            if (cNode.id === nodeId) {
                return cNode;
            ...
            console.log(cNode.id, nodeId);
            if (!cNode.children) continue;
            const result = findNodeInMyTreeData(nodeId, cNode.children);
            if (result) {
                return result;
            }
        }
...
const addNodesForParent = async (buttonNode: RGNode) => {
    const parentNode = buttonNode.lot.parent;
    console.log(' find ', parentNode.id);
    const nodeInTree = findNodeInMyTreeData(parentNode.id);
    ...
    for (let i = 0; i < randomNewNodesNum; i++) {
        ...
        nodeInTree.children.push({
            ...
            text: newNodeId,
            children: [
                { 'id': newNodeId + '-add-button', 'text': 'add node', disableNodeClickEffect: true, data: { myType: 'my-add-button'} },
            ]
        });
    }
    nodeInTree.children.sort((a, b) => (a.data && a.data.myType === 'my-add-button') ? 1 : -1);
...
import './relayout-after-add-nodes.scss'
...
const MyMainComponent = () => {
  ...
  const MyNodeSlot: React.FC<{ node: RGNode }> = useCallback(({ node }) => {
    return (
      <div className="w-full h-full">
        {node.data && node.data.myType === 'my-add-button' ? (
          <div className="my-add-button cursor-pointer" onClick={() => addNodesForParent(node)}>
