        ...
        <template #graph-plug>
          ...
          <div
            ...
            <svg width="100%" height="100%" viewBox="0 0 400 400">
              <g>
                <g fill="#000000">
                  <path class="c-svg-button" @click="clickNodeMenu('1', $event)" d="m9.99469,187.08997c0,-40.82082 27.52652,-97.86001 62.20722,-128.85359c32.50037,-29.06937 66.90853,-43.9133 114.603,-49.41106l7.15417,-0.82466l0,61.84972l0,61.91844l-4.49691,0c-23.5747,0 -58.05098,34.01735 -58.18725,57.3828l0,4.46692l-60.64011,0l-60.64011,0l0,-6.52858z"/>
                  <path class="c-svg-button" @click="clickNodeMenu('2', $event)" d="m269.58887,188.4644c-4.36064,-26.73282 -34.27188,-56.69558 -56.68828,-56.69558l-3.95183,0l0,-61.781l0,-61.71228l7.83552,0.68722c91.98219,8.52152 166.11303,82.60374 174.22109,174.00389l0.95389,10.6519l-60.77638,0l-60.77638,0l-0.81762,-5.15414z"/>
                  <path class="c-svg-button" @click="clickNodeMenu('3', $event)" d="m176.92533,393.32443c-89.32493,-11.68273 -166.93065,-94.35519 -166.93065,-177.92103l0,-6.66603l60.43571,0l60.50384,0l0.68135,5.97881c2.52099,23.09056 29.02549,50.99166 52.12324,55.04625c11.3104,1.99294 10.22024,-4.87926 10.22024,63.43033l0,61.09378l-6.47282,-0.20617c-3.54302,-0.13744 -8.31246,-0.41233 -10.56092,-0.75594z"/>
                  <path class="c-svg-button" @click="clickNodeMenu('4', $event)" d="m209.28944,332.98659l0.34067,-61.57483l7.49485,-1.64933c7.08604,-1.51188 11.51481,-3.22993 21.12184,-8.4528c6.06401,-3.22993 20.03168,-18.28003 23.43843,-25.22094c4.97385,-10.17084 7.22231,-15.94348 7.22231,-18.62364c0,-1.37444 0.40881,-3.91715 0.88575,-5.6352l0.81762,-3.09249l60.43571,0l60.50384,0l0,5.15414c-0.06813,86.31472 -81.62568,171.87351 -171.35942,179.77653l-11.24227,0.96211l0.34067,-61.64356z"/>
                </g>
              </g>
            </svg>
            <div style="height:100%;width:100%;position: absolute;left: 0px;top:0px;user-select: none;pointer-events: none;color: #ffffff;font-size: 22px;">
              <i class="el-icon-info" style="position: absolute;left:20px;top:20px;" />
              <i class="el-icon-copy-document" style="position: absolute;right:20px;top:20px;" />
              <i class="el-icon-male" style="position: absolute;left:20px;top:75px;transform: rotate(180deg);" />
              <i class="el-icon-circle-plus" style="position: absolute;right:20px;top:75px;" />
            </div>
...
import { RelationGraphInstance, RGJsonData } from 'relation-graph';
const graphOptions = {
  debug: false,
  defaultLineShape: 1,
...
export default {
  ...
  methods: {
    showGraph() {
      ...
      rgInstanceRef.setJsonData(graphJsonData, (graphInstance) => {
    ...
    updateNodeMenuPosition() {
      ...
      const graphInstance:RelationGraphInstance = this.$refs.graphRef.getInstance();
    ...
    copyNode() {
      ...
      const newNode = {
        ...
        borderColor: this.currentNode.borderColor,
        data: Object.assign({}, this.currentNode.data),
        force_weight: 20, // default is 1
        x: this.currentNode.x + 60,
        y: this.currentNode.y + 60
      };
      graphInstance.addNodes([newNode]);
      // graphInstance.addLines([{from: this.currentNode.id, to: newNodeId}]);
    ...
    showNodeInfo() {
      this.showNodeInfoCard = !this.showNodeInfoCard;
    },
    addChildNode() {
      const graphInstance = this.$refs.graphRef.getInstance();
      const newNodeId = graphInstance.generateNewNodeId();
      graphInstance.addNodes([{id: newNodeId, text: 'New Node', x: this.currentNode.x + 60, y: this.currentNode.y + 60}]);
      graphInstance.addLines([{from: this.currentNode.id, to: newNodeId}]);
      graphInstance.layouter.allLinks = graphInstance.getLinks();
      graphInstance.layouter.allNodes = graphInstance.getNodes();
      graphInstance.layouter.autoLayout();
    ...
    onChangeColor(v) {
      ...
      this.currentNode.borderColor = v;
      this.currentNode.color = v;
      this.currentNode.fontColor = v;
    },
    createLine(e) {
      const graphInstance = this.$refs.graphRef.getInstance();
...
import RelationGraphReact, {
  ...
  RGLink,
  RGOptions,
  JsonNode, Layout, RelationGraphInstance, RGNodeSlotProps
} from 'relation-graph-react';
// } from 'relation-graph-react';
import CircumIcon from "./RGDemoComponents/MyDemoIcons";
import demoData from './Demo4AdvDataFilterData.json';
import './scene-relationship-op.scss';
import {RelationGraphStoreContext} from "relation-graph-react";

const SceneRelationshipOp = () => {
  const graphRef = useRef<RelationGraphComponent | null>(null);
  const [showNodeMenu, setShowNodeMenu] = useState(false);
  const [nodeMenuPanel, setNodeMenuPanel] = useState({ x: 0, y: 0, width: 120, height: 120 });
  const [_forupdateview, updateMyCompView] = useState('');
  const [currentNode, setCurrentNode] = useState<RGNode|undefined>();
  const currentNodeRef = useRef<RGNode|undefined>();
  const [showNodeInfoCard, setShowNodeInfoCard] = useState(false);
  ...
  const showGraph = async () => {
    const graphJsonData: RGJsonData = demoData;
  ...
  const updateNodeMenuPosition = () => {
    console.log('updateNodeMenuPosition:', currentNodeRef.current);
    if (!currentNodeRef.current) return;
    const graphInstance = graphRef.current!.getInstance();
    if (graphInstance) {
      const rgNode = graphInstance.getNodeById(currentNodeRef.current!.id);
      console.log('updateNodeMenuPosition:', rgNode.x, rgNode.y);
      // const _base_position = graphInstance.getBoundingClientRect();
      const viewCoordinate = graphInstance.getClientCoordinateByCanvasCoordinate({
        x: rgNode.x + rgNode.el.offsetWidth / 2,
        y: rgNode.y + rgNode.el.offsetHeight / 2
      });
      setNodeMenuPanel({
        x: viewCoordinate.x - graphInstance.options.canvasOffset.x - nodeMenuPanel.width / 2,
        y: viewCoordinate.y - graphInstance.options.canvasOffset.y - nodeMenuPanel.height / 2,
  ...
  const onNodeClick = (nodeObject: RGNode, $event: RGUserEvent) => {
    currentNodeRef.current = nodeObject;
  ...
  const onCanvasClick = () => {
    ...
    setShowNodeInfoCard(false);
  };

  const clickNodeMenu = (menuId: string, e: React.MouseEvent) => {
    setShowNodeMenu(false);
    console.log('Node:', currentNode!.text, 'Menu Event:', menuId);
    if (menuId === '1') {
      setShowNodeInfoCard(!showNodeInfoCard);
    } else if (menuId === '2') {
      copyNode();
    } else if (menuId === '3') {
      createLine(e as RGUserEvent);
    } else if (menuId === '4') {
      addChildNode();
    }
  };

  const updateCurrentNodeText = (newText: string) => {
    if (currentNode) {
      currentNode!.text = newText;
      const graphInstance = graphRef.current!.getInstance();
      graphInstance.dataUpdated();
      updateMyCompView(newText);
    }
  }

  const copyNode = () => {
    const graphInstance = graphRef.current!.getInstance();
    if (graphInstance && currentNode) {
      const newNodeId = graphInstance.generateNewNodeId();
  ...
  const addChildNode = () => {
    const graphInstance = graphRef.current!.getInstance();
    const newNodeId = graphInstance.generateNewNodeId();
    graphInstance.addNodes([{ id: newNodeId, text: 'New Node', x: currentNode!.x + 60, y: currentNode!.y + 60 }]);
    graphInstance.addLines([{ from: currentNode!.id, to: newNodeId }]);
    graphInstance.layouter.allLinks = graphInstance.getLinks();
    graphInstance.layouter.allNodes = graphInstance.getNodes();
    graphInstance.layouter.autoLayout();
  };

  const updateNodeProperty = (key:string, newValue:string) => {
    if (currentNode) {
      const graphInstance = graphRef.current!.getInstance();
      if (key === 'text') currentNode.text = newValue;
      if (key === 'color') currentNode.color = newValue;
      if (key === 'borderColor') currentNode.borderColor = newValue;
      if (key === 'icon') currentNode.data.icon = newValue;
      setCurrentNode(currentNode);
      graphInstance.dataUpdated();
      updateMyCompView(newValue);
    }
  };

  const createLine = (e: RGUserEvent) => {
    const graphInstance = graphRef.current!.getInstance();
    graphInstance.startCreatingLinePlot(e, {
      template: {
        lineWidth: 2,
    ...
    <div>
      <div style={{ height: '100vh' }}>
        <RelationGraphReact
          ...
          onZoomEnd={updateNodeMenuPosition}
          onNodeDragEnd={updateNodeMenuPosition}
          onCanvasDragEnd={updateNodeMenuPosition}
          nodeSlot={({node}) => <div className="h-full">
          ...
          graphPlugSlot={<div>
            {
              showNodeMenu && <MyNodeMenu
                nodeMenuPanel={nodeMenuPanel}
                currentNodeText={currentNode!.text}
                onUpdateCurrentNodeText={updateCurrentNodeText}
                onClickNodeMenu={clickNodeMenu}
              />
            }
            {
              showNodeInfoCard && currentNode && <MyNodeInfoCard
...
interface MyNodeMenuProps{
  nodeMenuPanel: {x:number,y:number,width:number,height:number},
  currentNodeText: string,
  onUpdateCurrentNodeText: (newValue: string) => void,
  onClickNodeMenu: (menuId:string, e: React.MouseEvent) => void
}
const MyNodeMenu: React.FC<MyNodeMenuProps> = ({nodeMenuPanel, currentNodeText, onUpdateCurrentNodeText, onClickNodeMenu}) => {
  return <div
      style={{
        display: 'flex',
        alignItems: 'center',
      ...
      className="c-surround-menu-panel"
    >
      <svg width="100%" height="100%" viewBox="0 0 400 400">
        <g>
          <g fill="#000000">
            <path
              className="c-svg-button"
              d="m9.99469,187.08997c0,-40.82082 27.52652,-97.86001 62.20722,-128.85359c32.50037,-29.06937 66.90853,-43.9133 114.603,-49.41106l7.15417,-0.82466l0,61.84972l0,61.91844l-4.49691,0c-23.5747,0 -58.05098,34.01735 -58.18725,57.3828l0,4.46692l-60.64011,0l-60.64011,0l0,-6.52858"
              onClick={(e) => onClickNodeMenu('1', e)}
            />
            <path
              className="c-svg-button"
              d="m269.58887,188.4644c-4.36064,-26.73282 -34.27188,-56.69558 -56.68828,-56.69558l-3.95183,0l0,-61.781l0,-61.71228l7.83552,0.68722c91.98219,8.52152 166.11303,82.60374 174.22109,174.00389l0.95389,10.6519l-60.77638,0l-60.77638,0l-0.81762,-5.15414"
              onClick={(e) => onClickNodeMenu('2', e)}
            />
            <path
              className="c-svg-button"
              d="m176.92533,393.32443c-89.32493,-11.68273 -166.93065,-94.35519 -166.93065,-177.92103l0,-6.66603l60.43571,0l60.50384,0l0.68135,5.97881c2.52099,23.09056 29.02549,50.99166 52.12324,55.04625c11.3104,1.99294 10.22024,-4.87926 10.22024,63.43033l0,61.09378l-6.47282,-0.20617c-3.54302,-0.13744 -8.31246,-0.41233 -10.56092,-0.75594"
              onClick={(e) => onClickNodeMenu('3', e)}
            />
            <path
              className="c-svg-button"
              d="m209.28944,332.98659l0.34067,-61.57483l7.49485,-1.64933c7.08604,-1.51188 11.51481,-3.22993 21.12184,-8.4528c6.06401,-3.22993 20.03168,-18.28003 23.43843,-25.22094c4.97385,-10.17084 7.22231,-15.94348 7.22231,-18.62364c0,-1.37444 0.40881,-3.91715 0.88575,-5.6352l0.81762,-3.09249l60.43571,0l60.50384,0l0,5.15414c-0.06813,86.31472 -81.62568,171.87351 -171.35942,179.77653l-11.24227,0.96211l0.34067,-61.64356"
              onClick={(e) => onClickNodeMenu('4', e)}
            />
          </g>
        </g>
      </svg>
      <div style={{ height: '100%', width: '100%', position: 'absolute', left: 0, top: 0, userSelect: 'none', pointerEvents: 'none', color: '#ffffff', fontSize: '22px' }}>
        <div style={{ position: 'absolute', left: 20, top: 20 }}><CircumIcon color="#ffffff" size="20px" name="circle_info" /></div>
        <div style={{ position: 'absolute', right: 20, top: 20 }}><CircumIcon color="#ffffff" size="20px" name="link" /></div>
        <div style={{ position: 'absolute', left: 20, top: 75, transform: 'rotate(180deg)' }}><CircumIcon color="#ffffff" size="20px" name="maximize_1" /></div>
        <div style={{ position: 'absolute', right: 20, top: 75 }}><CircumIcon color="#ffffff" size="20px" name="circle_plus" /></div>
      </div>
      {<div className="c-current-node-text"><input value={currentNodeText}
                                                   onChange={(e) => onUpdateCurrentNodeText(e.target.value)}/></div>}
    </div>
};
interface MyNodeMenuProps{
  color: string,
  ...
  icon: string,
  borderColor: string,
  updateNodeProperty: (key:string, newValue:string) => void
}
const MyNodeInfoCard: React.FC<MyNodeMenuProps> = ({color, text, icon, borderColor, updateNodeProperty}) => {
  return <div className="c-operate-panels">
    {/* Node info card */}
    <div className="c-node-info-card" style={{ backgroundColor: color }}>
      ...
      Avatar:
      <img className="c-person-pic" src={icon} />
      <div className="c-node-options-panel">
        <div>Name:</div>
        <input value={text} onChange={(e) => {updateNodeProperty('text', e.target.value);}} style={{ width: '100%' }} />
        <div>Color:</div>
        <input value={borderColor} onChange={(e) => {updateNodeProperty('borderColor', e.target.value);}} style={{ width: '100%' }} />
        <div>Avatar:</div>
        <input value={icon} onChange={(e) => {updateNodeProperty('icon', e.target.value);}} style={{ width: '100%' }} />
      </div>
    </div>
    {/* Node options editor */}
  </div>
};
export default SceneRelationshipOp;
```

### `scene-relationship-op.scss`
