...
import VipMinPathSearchToolBar from "./VipMinPathSearchToolBar.vue";
const graphOptions = {
  allowShowMiniToolBar: false, // 禁用默认的工具栏，因为工具栏已经被重新定义了
...
export default {
  name: 'Demo',
  components: {VipMinPathSearchToolBar},
  data() {
    return {
      loading: true,
      fromNodeId: '',
...
```javascript
export class SeeksGraphDataBase {
  nodes
  edges
  ...
  getPath(currentNode, targetNode, nodesInPath) {
    ...
    if (currentNode.parentNode)
      return this.getPath(
        ...
        targetNode,
        nodesInPath.concat(currentNode.parentNode),
      )
  }
}
```

### `VipMinPathSearchToolBar.vue`

```javascript
<template>
  <div :style="{width:toolbarClosed?'65px':'350px'}" style="height:350px;padding-top:5px;position: absolute;right:20px;top:100px;z-index: 20;padding:20px;background-color: #ffffff;border:#efefef solid 1px;box-shadow: 0 3px 9px rgba(0,21,41,.08);">
    <VipMinPathSearchToolBarButtons />
    <div
        @click="toggleToolbar"
        style="position: absolute;z-index:40;left:0px;top:0px;width:20px;height:348px;background-color:#ffffff;color:#444444;cursor: pointer;display:flex;align-items: center;">
      <i :class="toolbarClosed?'el-icon-caret-left':'el-icon-caret-right'" />
    </div>
    <div v-if="!toolbarClosed" style="padding-left: 40px;">
      <div class="c-form-label" style="color: #333333;font-size: 14px;font-weight: bold;">
        节点间最短路径搜索
      </div>
      <div>
        <el-link type="warning" @click="test('N4', 'N9')">试着搜搜：高小凤，陈岩石</el-link>
      </div>
      <div>
        <el-link type="warning" @click="test('N10', 'N20')">试着搜搜：赵东来，蔡成功</el-link>
      </div>
      ...
      <el-select
          ...
          remote
          reserve-keyword
          placeholder="请输入开始节点的名称"
          :remote-method="fetchAllNodes"
          :loading="loading">
        <el-option
            v-for="item in nodes4Select"
            :key="item.value"
            :label="item.label"
            :value="item.value">
        </el-option>
      ...
      <div style="padding-top:10px;">
        <el-button icon="el-icon-search" size="mini" type="primary" @click="doSearch">搜索</el-button>
        <el-button icon="el-icon-search" size="mini" type="primary" @click="reset">重置</el-button>
      </div>
    </div>
  </div>
</template>
...
// 如果您没有在main.js文件中使用Vue.use(RelationGraph); 就需要使用下面这一行代码来引入relation-graph
// import RelationGraph from 'relation-graph';
import {SeeksGraphDataBase} from './VipMinPathSearch.js'
import VipMinPathSearchToolBarButtons from "./VipMinPathSearchToolBarButtons.vue";
export default {
  name: 'VipMinPathSearchToolBar',
  components: {VipMinPathSearchToolBarButtons},
  data() {
    return {
      toolbarClosed: false,
      loading: true,
  ...
  methods: {
    ...
    fetchAllNodes(keywords) {
      this.loading = true;
      const graphInstance =  this.relationGraph;
      this.nodes4Select = graphInstance.getNodes().filter(node => {
        return node.text.includes(keywords)
      }).map(node => {
        return { value: `${node.id}`, label: `name:${node.text}` };
      });
      this.loading = false;
    },
    doSearch() {
      if (!this.fromNodeId) {
        return this.$message({type:'error', message:'请输入起点!'})
      }
      if (!this.toNodeId) {
        return this.$message({type:'error', message:'请输入终点!'})
      }
      const graphInstance =  this.relationGraph;
      const graphDb = new SeeksGraphDataBase(); // 每次搜索都需要创建此对象
      graphDb.loadDataFromRelationGraph(graphInstance)
      const nodesInMinPath = graphDb.findMinPath(this.fromNodeId, this.toNodeId);
      if (!nodesInMinPath) {
        return this.$message({type:'error', message:'找不到起点与终点的关联!'})
      }
      console.log('nodesInMinPath:', nodesInMinPath);
      // nodesInMinPath是一个数组，数组中包含了最短路径上所有的节点
      this.drawMinPath(nodesInMinPath);
    },
    // 将最短路径上的所有节点和节点之间的关系高亮显示在图谱上
    drawMinPath(nodesInMinPath) {
      // 清除之前的最短路径显示状态（如果有的话）
      this.clearMinPath();
      // 循环遍历所有节点，如果节点包含在路径上，什么都不做，如果不在，则将节点的透明度降低，显示为置灰的效果
      this.relationGraph.getNodes().forEach(node => {
        if (!nodesInMinPath.includes(node.id)) {
          node.opacity = 0.2;
        }
      })
      // 循环遍历所有节点，如果线条包含在路径上，则修改线条的颜色为红色并加粗，如果不在，则将线条的透明度降低，显示为置灰的效果
      this.relationGraph.getLinks().forEach(link => {
        link.relations.forEach(line => {
          if (!nodesInMinPath.includes(link.fromNode.id) || !nodesInMinPath.includes(link.toNode.id)) {
            // 如果线条不在路径上，则将线条的透明度降低，显示为置灰的效果
            line.opacity = 0.2;
          } else { // 如果线条包含在路径上，则修改线条的颜色为红色并加粗
            line.data.orignColor = line.color // 记录原本的颜色，在取消高亮时还原用
            line.color = '#ff0000';
            line.lineWidth = 3;
          }
        })
    ...
    clearMinPath() {
      ...
      this.relationGraph.getLinks().forEach(link => {
        link.relations.forEach(line => {
          ...
          line.opacity = 1;
          line.lineWidth = 1;
          line.color = line.data.orignColor; // 还原为高亮之前原本的颜色
        })
      })
    },
    reset() {
...
### `VipMinPathSearchToolBarButtons.vue`

```javascript
<template>
  <div class="c-mini-toolbar" style="height:348px;border-left:#efefef solid 1px;">
    <div style="height:45px;">
      <img src="/images/rg-logo.png" style="width: 100%;height:100%;">
    </div>
    ...
    <div v-if="options.allowShowZoomMenu" style="float:left;margin-top:0px;height:20px;width:100%;border-top:0px;border-bottom:0px;background-color: #ffffff;color: #262626;font-size: 10px;background-color: #efefef;text-align: center;line-height: 20px;" @dblclick="zoomToFit">{{ options.canvasZoom }}%</div>
    ...
    <div v-if="options.allowShowDownloadButton" class="c-mb-button">
      ...
      <div :style="{width:downloadPanelWidth+'px','margin-left':(downloadPanelWidth*-1)+'px'}" class="c-mb-child-panel">
        ...
        <div class="c-mb-button c-mb-button-c" style="width: 50px;" @click="onDownloadExcel()">
          <svg class="rg-icon" aria-hidden="true"><use xlink:href="#icon-ziyuan"></use></svg>
...
export default {
  name: 'VipMinPathSearchToolBarButtons',
  data() {
    return {
      downloadPanelWidth: 156
    };
  },
  inject: ['graph', 'graphInstance'],
  computed: {
...
<template>
  <div :style="{width:toolbarClosed?'65px':'350px'}" style="height:350px;padding-top:5px;position: absolute;right:20px;top:100px;z-index: 20;padding:20px;background-color: #ffffff;border:#efefef solid 1px;box-shadow: 0 3px 9px rgba(0,21,41,.08);">
    ...
    <div
      ...
      style="position: absolute;z-index:40;left:0px;top:0px;width:20px;height:100%;background-color:#f3ba91;color:#444444;cursor: pointer;display:flex;align-items: center;"
      @click="toggleToolbar">
      <i :class="toolbarClosed?'el-icon-caret-left':'el-icon-caret-right'" />
    </div>
    <div v-if="!toolbarClosed" style="padding-left: 40px;">
      ...
      <div>
        <el-link type="warning" @click="test('N4', 'N9')">Try searching: Gao Xiaofeng, Chen Yanshi</el-link>
      </div>
      <div>
        <el-link type="warning" @click="test('N10', 'N20')">Try searching: Zhao Donglai, Cai Chenggong</el-link>
      </div>
      <div class="c-form-label">
        Start Node:
      </div>
      <el-select
        ...
        placeholder="Please enter the name of the start node"
        :remote-method="fetchAllNodes"
        :loading="loading">
        <el-option
      ...
      <div>
        <el-select
          ...
          placeholder="Please enter the name of the end node"
          :remote-method="fetchAllNodes"
          :loading="loading">
          <el-option
...
import { SeeksGraphDataBase } from './VipMinPathSearch';
import VipMinPathSearchToolBarButtons from './VipMinPathSearchToolBarButtons.vue';
import {ElMessage} from 'element-plus';
import {RelationGraphFinal} from "relation-graph/types/types/models/RelationGraphFinal";

const toolbarClosed = ref(false);
const loading = ref(true);
const fromNodeId = ref('');
const toNodeId = ref('');
const nodes4Select = ref([]);

const graph = inject(graphKey);
const options = computed(() => {
    return graph && graph.options;
});
const graphInstance = computed<RelationGraphFinal>(() => {
    return graph && graph.instance;
});

const toggleToolbar = () => {
    toolbarClosed.value = !toolbarClosed.value;
};

const fetchAllNodes = (keywords: string) => {
    loading.value = true;
    nodes4Select.value = graphInstance.value.getNodes().filter(node => {
        return node.text.includes(keywords);
    }).map(node => {
        return { value: `${node.id}`, label: `name:${node.text}` };
    });
...
const doSearch = () => {
    if (!fromNodeId.value) {
        return ElMessage({type:'error', message:'Please enter the start node!'});
    }
    if (!toNodeId.value) {
        return ElMessage({type:'error', message:'Please enter the end node!'});
    }
    const graphDb = new SeeksGraphDataBase();
    graphDb.loadDataFromRelationGraph(graphInstance.value);
    const nodesInMinPath = graphDb.findMinPath(fromNodeId.value, toNodeId.value);
    if (!nodesInMinPath) {
        return ElMessage({type:'error', message:'Cannot find the association between the start node and the end node!'});
    }
    console.log('nodesInMinPath:', nodesInMinPath);
    drawMinPath(nodesInMinPath);
};

const drawMinPath = (nodesInMinPath) => {
    clearMinPath();
...
```javascript
<template>
  <div v-if="options" class="c-mini-toolbar" style="height:100%;border-left:#efefef solid 1px;">
    <div style="height:45px;display: flex;place-items: center;justify-content: center;">
      <img src="/images/rg-logo.png" style="height:100%;">
    </div>
    <div class="c-mb-button" style="margin-top: 0px;" @click="relationGraph.fullscreen();">
      <svg class="rg-icon" aria-hidden="true"><use xlink:href="#icon-resize-"></use></svg>
      <div class="c-mb-text">{{ options.fullscreen?'Exit':'Fullscreen' }}</div>
    </div>
    <div v-if="options.allowShowZoomMenu" class="c-mb-button" @click="relationGraph.zoom(20)">
      <svg class="rg-icon" aria-hidden="true"><use xlink:href="#icon-fangda"></use></svg>
      <div class="c-mb-text">Zoom In</div>
    </div>
    <div v-if="options.allowShowZoomMenu" style="margin-top:0px;height:20px;width:100%;border-top:0px;border-bottom:0px;background-color: #ffffff;color: #262626;font-size: 10px;background-color: #efefef;text-align: center;line-height: 20px;" @dblclick="zoomToFit">{{ options.canvasZoom }}%</div>
    <div v-if="options.allowShowZoomMenu" class="c-mb-button" style="margin-top:0px;" @click="relationGraph.zoom(-20)">
      <svg class="rg-icon" aria-hidden="true"><use xlink:href="#icon-suoxiao"></use></svg>
      <div class="c-mb-text">Zoom Out</div>
    </div>
    <div v-if="options.layouts.length > 1" class="c-mb-button">
      <svg class="rg-icon" aria-hidden="true"><use xlink:href="#icon-yuanquanfenxiang"></use></svg>
      ...
      <div :style="{width:(options.layouts.length * 70 + 6)+'px','margin-left':(options.layouts.length * -70 - 5)+'px'}" class="c-mb-child-panel">
        <div v-for="thisLayoutSetting in options.layouts" :key="thisLayoutSetting.label" class="c-mb-button c-mb-button-c" :class="{'c-mb-button-on':options.layoutLabel===thisLayoutSetting.label}" style="width: 70px;" @click="switchLayout(thisLayoutSetting)">
          <svg class="rg-icon" aria-hidden="true"><use xlink:href="#icon-yuanquanfenxiang"></use></svg>
          <div class="c-mb-text">{{ thisLayoutSetting.label }}</div>
        </div>
      </div>
    </div>
    <div v-if="options.allowAutoLayoutIfSupport && options.isNeedShowAutoLayoutButton" :title="options.autoLayouting?'Click to stop auto layout':'Click to start auto layout'" :class="{'c-mb-button-on':options.autoLayouting}" class="c-mb-button" @click="toggleAutoLayout">
      <svg v-if="!options.autoLayouting" class="rg-icon" aria-hidden="true"><use xlink:href="#icon-zidong"></use></svg>
      <svg v-else class="c-loading-icon rg-icon" aria-hidden="true"><use xlink:href="#icon-lianjiezhong"></use></svg>
      <div class="c-mb-text">Auto</div>
    </div>
    <div v-if="options.allowShowRefreshButton" class="c-mb-button" @click="refresh">
      <svg class="rg-icon" aria-hidden="true"><use xlink:href="#icon-ico_reset"></use></svg>
    ...
    <div v-if="options.allowShowDownloadButton" class="c-mb-button">
      <svg class="rg-icon" aria-hidden="true"><use xlink:href="#icon-ziyuan"></use></svg>
      <div class="c-mb-text">Download</div>
      <div :style="{width:downloadPanelWidth+'px','margin-left':(downloadPanelWidth*-1)+'px'}" class="c-mb-child-panel">
        <div class="c-mb-button c-mb-button-c" style="width: 50px;" @click="relationGraph.downloadAsImage('png')">
          <svg class="rg-icon" aria-hidden="true"><use xlink:href="#icon-tupian"></use></svg>
          <span class="c-mb-text">PNG</span>
...
const downloadPanelWidth = ref(156);
const graph = inject(graphKey);
const options = computed(() => {
    return graph && graph.options;
});
const refresh = () => {
    const graphInstance = graph.instance;
    if (graphInstance) {
...
const onDownloadExcel = () => {
    const graphInstance = graph.instance;
    if (graphInstance) {
        graphInstance.listeners.onDownloadExcel();
    }
};
...
import VipMinPathSearchToolBar from './VipMinPathSearchToolBar'
import './find-min-path.scss'
    ...
    <div>
      <div style={{ height: '100vh' }}>
        <RelationGraph
          ...
          nodeSlot={NodeSlot}
          toolBarSlot={VipMinPathSearchToolBar}
        ></RelationGraph>
      </div>
    </div>
  )
...
import { SeeksGraphDataBase } from './VipMinPathSearch';
import VipMinPathSearchToolBarButtons from './VipMinPathSearchToolBarButtons';
import {ElMessage, MyButton, MyCheckBox, MyLinkButton, MySelector} from "./RGDemoComponents/MyUIComponents";
import './VipMinPathSearchToolBar.scss';


const VipMinPathSearchToolBar: React.FC<{relationGraph: RelationGraphInstance}> = ({relationGraph}) => {
  // if (!relationGraph) return;
  const graphInstance = relationGraph;
  const [toolbarClosed, setToolbarClosed] = useState(false);
  const [fromNodeId, setFromNodeId] = useState('');
  const [toNodeId, setToNodeId] = useState('');
  const [nodes4Select, setNodes4Select] = useState([]);
  const toggleToolbar = () => {
    setToolbarClosed(!toolbarClosed);
  };

  const createSelectItems = () => {
    const nodes = graphInstance.getNodes().map((node: RGNode) => {
      return { value: `${node.id}`, text: `${node.text}` };
    });
    setNodes4Select(nodes);
  };
  ...
  const doSearch = () => {
    ...
    const graphDb = new SeeksGraphDataBase();
    graphDb.loadDataFromRelationGraph(graphInstance);
    const nodesInMinPath = graphDb.findMinPath(fromNodeId, toNodeId);
    if (!nodesInMinPath) {
      return ElMessage({ type: 'error', message: 'Cannot find the association between the start node and the end node!' });
    }
    console.log('nodesInMinPath:', nodesInMinPath);
  ...
  const drawMinPath = (nodesInMinPath: string[]) => {
    clearMinPath();
  ...
  return (
    <div style={{ width: toolbarClosed ? '95px' : '500px', height: '400px', paddingTop: '5px', position: 'absolute', right: '20px', top: '100px', zIndex: 20, padding: '20px', backgroundColor: '#ffffff', border: '#efefef solid 1px', boxShadow: '0 3px 9px rgba(0,21,41,.08)' }}>
      <VipMinPathSearchToolBarButtons relationGraph={relationGraph} />
      <div style={{ position: 'absolute', zIndex: 40, left: '0px', top: '0px', width: '20px', height: '100%', backgroundColor: '#f3ba91', color: '#444444', cursor: 'pointer', display: 'flex', alignItems: 'center' }} onClick={toggleToolbar}>
        <i className={toolbarClosed ? 'el-icon-caret-left' : 'el-icon-caret-right'} />
      </div>
      {!toolbarClosed && (
        <div style={{ paddingLeft: '20px', paddingRight: '80px' }}>
          <div className="c-form-label" style={{ color: '#333333', fontSize: '14px', fontWeight: 'bold' }}>
            Node Path Search
          </div>
          <div>
            <MyLinkButton onClick={() => test('N4', 'N9')}>
              Try searching: Gao Xiaofeng, Chen Yanshi
            </MyLinkButton>
          </div>
          <div>
            <MyLinkButton onClick={() => test('N10', 'N20')}>
              Try searching: Zhao Donglai, Cai Chenggong
            </MyLinkButton>
          </div>
          <div className="c-form-label">Start Node:</div>
          <div className="h-16 overflow-auto border-gray-500 border rounded-lg">
            <MyCheckBox
              data={nodes4Select}
              currentValue={fromNodeId}
              onChange={(newValue: string, label: string) => {
                setFromNodeId(newValue)
              }}
            />
          </div>
          <div className="c-form-label">End Node:</div>
          <div className="h-16 overflow-auto border-gray-500 border rounded-lg">
            <MyCheckBox
              data={nodes4Select}
              currentValue={toNodeId}
              onChange={(newValue: string, label: string) => {
                setToNodeId(newValue)
              }}
            />
          </div>
          <div style={{ paddingTop: '10px' }}>
            {/*<MyButton onClick={doSearch}>*/}
            {/*  Search*/}
            {/*</MyButton>*/}
...
export default VipMinPathSearchToolBar;
```

### `VipMinPathSearchToolBarButtons.scss`
...
### `VipMinPathSearchToolBarButtons.tsx`
...
import './VipMinPathSearchToolBarButtons.scss'

interface VipMinPathSearchToolBarButtonsProps {
  relationGraph: RelationGraphInstance;
}

const VipMinPathSearchToolBarButtons: React.FC<VipMinPathSearchToolBarButtonsProps> = ({
  relationGraph,
}) => {
  const graphInstance = relationGraph
  ...
  const setZoom = async (newValue: number) => {
    await graphInstance.zoom(newValue)
  }

  const downloadImage = async () => {
    await graphInstance.downloadAsImage('png')
  }
  ...
  return (
    <div>
      {options && (
        <div className="c-mini-toolbar" style={{ height: '100%', borderLeft: '#efefef solid 1px' }}>
          <div
            style={{
              height: '65px',
              display: 'flex',
          ...
          >
            <img
              src="https://www.relation-graph.com/images/rg-logo.png"
              style={{ height: '100%' }}
            />
          </div>
          <div className="c-mb-button" style={{ marginTop: '0px' }} onClick={() => fullscreen()}>
            <svg className="rg-icon" aria-hidden="true">
              <use xlinkHref="#icon-resize-"></use>
            </svg>
            <div className="c-mb-text">{options.fullscreen ? 'Exit' : 'Fullscreen'}</div>
          </div>
          {options.allowShowZoomMenu && (
            <div className="c-mb-button" onClick={() => setZoom(20)}>
              <svg className="rg-icon" aria-hidden="true">
                <use xlinkHref="#icon-fangda"></use>
              </svg>
              <div className="c-mb-text">Zoom In</div>
            </div>
          )}
          {options.allowShowZoomMenu && (
            <div
          ...
          {options.allowShowZoomMenu && (
            <div className="c-mb-button" style={{ marginTop: '0px' }} onClick={() => setZoom(-20)}>
              <svg className="rg-icon" aria-hidden="true">
                <use xlinkHref="#icon-suoxiao"></use>
              </svg>
              <div className="c-mb-text">Zoom Out</div>
            </div>
          )}
          {options.allowAutoLayoutIfSupport && options.isNeedShowAutoLayoutButton && (
            ...
            >
              ...
              <div className="c-mb-text">Auto</div>
            </div>
          )}
          {options.allowShowRefreshButton && (
            <div className="c-mb-button" onClick={refresh}>
              <svg className="rg-icon" aria-hidden="true">
                <use xlinkHref="#icon-ico_reset"></use>
              </svg>
              <div className="c-mb-text">Refresh</div>
            </div>
          )}
          {options.allowShowDownloadButton && (
            <div className="c-mb-button" onClick={downloadImage}>
              <svg className="rg-icon" aria-hidden="true">
                <use xlinkHref="#icon-ziyuan"></use>
              </svg>
              <div className="c-mb-text">Image</div>
            </div>
          )}
        </div>
      )}
...
export default VipMinPathSearchToolBarButtons
```
