    ...
    <div style="height:calc(100vh);">
      <RelationGraph
        ...
        <template #graph-plug>
          <MyDemoPanel>
            <div class="c-content">
              这个示例通过插槽，以普通div元素的方式自定义了一个图文结合的水印，里面还设置了动态内容（时间）。
            </div>
            <div class="c-content">
              前景水印可以设置为不显示，但却能被绘制到导出的图片中。
            </div>
            <div class="c-content">
    ...
    <div style="height:calc(100vh);">
      <RelationGraph ref="$graphRef" :options="graphOptions">
        <template #graph-plug>
          <MyDemoPanel>
            <div class="c-content">
              This example customizes a watermark with a combination of text and image using a slot in a regular div element.
            </div>
            <div class="c-content">
...
const downloadImageByRGApi = () => {
    graphInstance.value.downloadAsImage('png', 'your-filename.png');
};
...
import './foreground-watermark.scss';

const MyCustomGraphWithWatermark = () => {
...
export default MyCustomGraphWithWatermark;
```

### `foreground-watermark.scss`
