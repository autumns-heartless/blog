...
import './scene-relationship.scss';
import {MySelector} from "./RGDemoComponents/MyUIComponents";

const MyComponent0001 = () => {
  const graphRef = useRef<RelationGraphComponent|null>(null);
  const [checked_sex, setChecked_sex] = useState('');
  const [checked_isgoodman, setChecked_isgoodman] = useState('');
  const [rel_checkList, setRel_checkList] = useState(['teacher-student', 'superior-subordinate', 'relative', 'lover', 'friend', 'couple', 'collusion', 'corruption', 'report']);
  const all_rel_type = ['teacher-student', 'superior-subordinate', 'relative', 'lover', 'friend', 'couple', 'collusion', 'corruption', 'report'];
  ...
  const toggleRelTypeItem = (item) => {
    console.log('toggleRelTypeItem:', item);
    if (rel_checkList.includes(item)) {
      rel_checkList.splice(rel_checkList.indexOf(item), 1);
      setRel_checkList([].concat(rel_checkList));
    } else {
      rel_checkList.push(item);
      setRel_checkList([].concat(rel_checkList));
    }
  }
  const doFilter = () => {
    ...
    _all_nodes.forEach(thisNode => {
      let _isHideThisLine = false;
      if (checked_sex !== '') {
        if (thisNode.data!['sexType'] !== checked_sex) {
          _isHideThisLine = true;
        }
      }
      if (checked_isgoodman !== '') {
        if (thisNode.data!['isGoodMan'] !== checked_isgoodman) {
          _isHideThisLine = true;
        }
      }
      thisNode.opacity = _isHideThisLine ? 0.1 : 1;
    ...
    _all_links.forEach(thisLink => {
      thisLink.relations.forEach(thisLine => {
        let hideLine = false;
        if (rel_checkList.indexOf(thisLine.data!['type']) === -1) {
          hideLine = true;
          console.log('Hide line:', thisLine);
        }
        // thisLine.isHide = hideLine;
        thisLine.opacity = hideLine ? 0.1 : 1;
      });
    });
    graphInstance?.dataUpdated();
  };
  ...
  useEffect(() => {
    doFilter();
  }, [checked_sex, checked_isgoodman, rel_checkList]);
  return (
    <div>
      <div style={{ marginTop: 0, width: 'calc(100% - 10px)', height: '100vh' }}>
        <div style={{width: '700px'}} className="rounded-lg absolute left-20 top-20 z-20 p-4 bg-white border-solid border-2 border-black shadow-lg">
          <div className="flex justify-center place-items-center gap-4">
            ...
            <div>
              <MySelector
                ...
                currentValue={checked_sex}
                onChange={(newValue: string, label) => { setChecked_sex((newValue)); }}
              />
            </div>
            <div>
              <MySelector
                ...
                currentValue={checked_isgoodman}
                onChange={(newValue: string, label) => { setChecked_isgoodman(newValue); }}
              />
            </div>
          </div>
          <div>
            <div>Relation Filter:</div>
            <div className="flex gap-2 flex-wrap">
              {
                all_rel_type.map(thisItem =>
                  <div
                    key={thisItem}
                    className={`px-2 py-1 flex justify-center place-items-center rounded-sm text-sm cursor-pointer  hover:bg-gray-300 ${rel_checkList.includes(thisItem) ? 'text-blue-600 font-bold':'text-gray-500'}`}
                    onClick={()=>{toggleRelTypeItem(thisItem);}}
                  >
                    <CircumIcon name="circle_check" size={18} />
                    {thisItem}
                  </div>
                )
              }
...
export default MyComponent0001;
```

### `scene-relationship.scss`
