...
<template>
  <div>
    <div style="height:calc(100vh);">
      ...
      >
        <template #node="{node}">
          <div class="c-my-node-icon"><!---------------- if node a ---------------->
            <i style="font-size: 30px;" :class="node.data.myicon" />
          </div>
        </template>
        <template #graph-plug>
          <div class="c-my-panel">
            <div class="c-option-name">操作说明：</div>
            <div class="c-option-name">方式1：按住Shift开始框选</div>
            <div class="c-option-name">方式2：禁用拖拽画布来支持框选</div>
            <div class="c-option-name">
              <button class="c-my-button" :class="{'c-my-button-checked':myEnableSelectionMode}" @click="toggleDisableDragCanvas">
                {{myEnableSelectionMode ? '关闭框选模式':'开启框选模式'}}
              </button>
            </div>
            <div class="c-option-name">当前已选择节点个数：{{selectedNodes.length}}</div>
          </div>
        </template>
...
import {RGLink} from "relation-graph";
...
export default {
  ...
  methods: {
    ...
    toggleDisableDragCanvas() {
      this.myEnableSelectionMode = !this.myEnableSelectionMode;
      const graphInstance = this.$refs.graphRef.getInstance();
      graphInstance.options.selectionMode = this.myEnableSelectionMode;
    },
    onCanvasSelectionEnd(selectionView, $event) {
      ...
      const startCoordinateOnCanvas = graphInstance.getCanvasCoordinateByClientCoordinate({
        x: selectionView.x + _base_position.x,
        y: selectionView.y + _base_position.y
      });
      const endCoordinateOnCanvas = graphInstance.getCanvasCoordinateByClientCoordinate({
    ...
    updateSelectedNodes() {
      ...
      const graphInstance = this.$refs.graphRef.getInstance();
      graphInstance.getNodes().forEach(node => {
        if (node.selected) this.selectedNodes.push(node);
      });
    },
  }
};
...
::v-deep .rel-map {
  ...
  background-image: linear-gradient(90deg,rgba(0,0,0,.1) 2px,transparent 0),linear-gradient(180deg,rgba(0,0,0,.1) 2px,transparent 0);
  .rel-node-selected {
    border: 0px;
      ...
      >
        <template #node="{node}">
          <div class="c-my-node-icon">
              <CircumIcon size="50px" name="cloud_on"/>
          </div>
        </template>
        <template #graph-plug>
          <div class="c-my-panel">
            <div class="c-option-name">Operation Instructions:</div>
            <div class="c-option-name">Method 1: Hold Shift to select</div>
            <div class="c-option-name">Method 2: Disable drag canvas to support selection</div>
            <div class="c-option-name">
              <button class="c-my-button" :class="{'c-my-button-checked':enableSelectionMode}" @click="toggleDisableDragCanvas">
                {{enableSelectionMode ? 'Disable SelectionMode' : 'Enable SelectionMode'}}
              </button>
            </div>
            <div class="c-option-name">Number of selected nodes: {{selectedNodes.length}}</div>
          </div>
        </template>
        <template #canvas-plug>
          <!--- Place elements that are not allowed to be dragged here --->
        </template>
      </RelationGraph>
    </div>
  </div>
...
import { RGNodesAnalyticUtils, RGJsonData, RGNode, RGLine, RGLink, RGUserEvent, RelationGraphComponent, RGOptions } from 'relation-graph-vue3';
import CircumIcon from "@klarr-agency/circum-icons-vue"; // Vue

const { RGNodesAnalytic } = RGNodesAnalyticUtils;

const enableSelectionMode = ref(true);
...
const showGraph = async() => {
    const __graph_json_data: RGJsonData = {
        rootId: '2',
        nodes: [
            // Note: Your custom properties need to be placed in the data tag like this, otherwise the data will be lost
...
const toggleDisableDragCanvas = () => {
    enableSelectionMode.value = !enableSelectionMode.value;
    const graphInstance = graphRef.value?.getInstance();
    if (graphInstance) {
        graphInstance.options.selectionMode = enableSelectionMode.value;
    }
};
...
const updateSelectedNodes = () => {
    ...
    if (graphInstance) {
        graphInstance.getNodes().forEach((node: RGNode) => {
            if (node.selected) selectedNodes.value.push(node);
        });
    }
};
</script>
...
const MyComponent = () => {
  ...
  const [enableSelectionMode, setEnableSelectionMode] = useState(true)
  ...
  const toggleDisableDragCanvas = () => {
    ...
    if (graphInstance) {
      graphInstance.options.selectionMode = enableSelectionMode
  ...
  const updateSelectedNodes = () => {
    ...
    if (graphInstance) {
      graphInstance.getNodes().forEach((node: RGNode) => {
        if (node.selected) newSelectedNodes.push(node)
      })
  ...
  const NodeSlot: React.FC<{ node: RGNode }> = ({ node }) => {
    return <CircumIcon size="50px" name="cloud_on" />
  }
      ...
      <div style={{ height: '100vh' }}>
        <MyDemoPanel>
          <div className="c-my-panel">
            <div className="c-option-name">Operation Instructions:</div>
            <div className="c-option-name">Method 1: Hold Shift to select</div>
            <div className="c-option-name">Method 2: Disable drag canvas to support selection</div>
            <div className="c-option-name">
              <button
                className={`c-my-button ${enableSelectionMode ? 'c-my-button-checked' : ''}`}
                style={{ width: '200px' }}
                onClick={() => {
                  setEnableSelectionMode(!enableSelectionMode)
                }}
              >
                {enableSelectionMode ? 'Disable SelectionMode' : 'Enable SelectionMode'}
              </button>
            </div>
            <div className="c-option-name">Number of selected nodes: {selectedNodes.length}</div>
          </div>
        </MyDemoPanel>
        <RelationGraph
          ref={graphRef}
