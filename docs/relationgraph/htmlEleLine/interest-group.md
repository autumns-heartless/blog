  ...
  <div>
    <div style="height:calc(100vh);">
      <RelationGraph
        ...
        <div style="width: 300px;position: absolute;left: 600px;top: 0px;">
          <div>Class Members</div>
          <div>
            <template v-for="member of classMembers">
              <div :key="member.name" :id="`member-${member.name}`" class="c-i-member" :class="{'c-i-member-checked': checkedMemberId === member.name}" @click="onMemberrClick(member.name)">
                <div class="c-i-name">{{member.name}}</div>
                <div style="width: 50px;font-size: 12px;line-height: 20px;">Joined:</div>
                <div class="c-i-count">{{member.joinedGroups.length}}</div>
              </div>
            </template>
          </div>
        </div>
        <template #canvas-plug><!-- behind others-->
          <div style="z-index:10;position: absolute;left: -700px;top: 100px;height: 800px;width:800px;background-image: url('/images/school-map.png');background-repeat: no-repeat;">
...
export default {
  ...
  methods: {
    showGraph() {
      ...
      this.classMembers = [
        { name: 'Tom', joinedGroups: ['a', 'c', 'd'] },
        { name: 'Emma', joinedGroups: ['b', 'e', 'f'] },
        { name: 'Liam', joinedGroups: ['a', 'b', 'd'] },
        { name: 'Olivia', joinedGroups: ['c', 'e', 'f'] },
        { name: 'Noah', joinedGroups: ['a', 'c', 'e', 'f'] },
        { name: 'Ava', joinedGroups: ['b', 'd'] },
        { name: 'Sophia', joinedGroups: ['c', 'e'] },
        { name: 'Jackson', joinedGroups: ['a', 'b', 'd', 'f'] },
        { name: 'Isabella', joinedGroups: ['c', 'e'] },
        { name: 'Lucas', joinedGroups: ['a', 'b', 'f'] },
        { name: 'Mia', joinedGroups: ['c', 'd'] },
        { name: 'Jacob', joinedGroups: ['a', 'e'] },
        { name: 'Charlotte', joinedGroups: ['b', 'd', 'f'] },
        { name: 'Ethan', joinedGroups: ['c', 'e', 'f'] },
        { name: 'Amelia', joinedGroups: ['a', 'b', 'd'] }
      ];
    ...
    onGroupClick(groupId) {
      ...
      this.classMembers.forEach(member => {
        if (member.joinedGroups.includes(this.checkedGroupId)) {
          htmlElementLines.push({
            from: 'member-' + member.name,
            to: 'group-' + this.checkedGroupId,
            color: 'rgba(29,169,245,0.76)', // rgba(159,23,227,0.65)
            lineShape: 6
          });
        }
      });
    ...
    onMemberrClick(memberName) {
      console.log('onMemberrClick');
      this.checkedGroupId = '';
      this.checkedMemberId = memberName;
      const htmlElementLines = [];
      this.classMembers.find(member => member.name === memberName).joinedGroups.forEach(groupId => {
        htmlElementLines.push({
    ...
    <div style="height:calc(100vh);">
      <RelationGraph ref="graphRef" :options="graphOptions">
        ...
        <div style="width: 300px;position: absolute;left: 600px;top: 0px;">
          <div>Class Members</div>
          <div>
            <template v-for="member of classMembers" :key="member.name">
              <div

                :id="`member-${member.name}`"
                class="c-i-member"
                :class="{'c-i-member-checked': checkedMemberId === member.name}"
                @click="onMemberClick(member.name)"
              >
                <div class="c-i-name">{{ member.name }}</div>
                <div style="width: 50px;font-size: 12px;line-height: 20px;">Joined:</div>
                <div class="c-i-count">{{ member.joinedGroups.length }}</div>
          ...
          <div style="z-index:10;position: absolute;left: -700px;top: 100px;height: 800px;width:800px;background-image: url('/images/school-map.png');background-repeat: no-repeat;">
            <div style="position: relative;">
              <div
                ...
                v-for="group of interestGroups"
                :key="group.groupId"
                :id="`location-${group.groupId}`"
                class="c-i-location"
                :class="{'c-i-location-active': checkedGroupId === group.groupId}"
...
const onGroupClick = (groupId: string) => {
    ...
    const htmlElementLines = [];
    classMembers.value.forEach(member => {
        if (member.joinedGroups.includes(checkedGroupId.value)) {
            htmlElementLines.push({
                from: 'member-' + member.name,
                to: 'group-' + checkedGroupId.value,
...
const onMemberClick = (memberName: string) => {
    ...
    checkedMemberId.value = memberName;
    const htmlElementLines = [];
    classMembers.value.find(member => member.name === memberName)?.joinedGroups.forEach(groupId => {
        htmlElementLines.push({
            from: 'member-' + memberName,
            to: 'group-' + groupId,
            color: 'rgba(159,23,227,0.65)',
...
const InterestGroup = () => {
  ...
  const initGraph = async () => {
    ...
    if (graphInstance) {
      graphInstance.addNodes([{ id: 'fake-root', text: '', opacity: 0 }])
      setTimeout(() => {
        onGroupClick('a')
      }, 200)
    }
  ...
  const onGroupClick = (groupId: string) => {
    ...
    classMembers.forEach((member) => {
      if (member.joinedGroups.includes(checkedGroupId.current)) {
        htmlElementLines.push({
          from: 'member-' + member.name,
          to: 'group-' + checkedGroupId.current,
          color: 'rgba(29,169,245,0.76)',
          lineShape: 6,
        })
      }
  ...
  const onMemberClick = (memberName: string) => {
    checkedGroupId.current = ''
    checkedMemberId.current = memberName
    const htmlElementLines = []
    classMembers
  ...
  useEffect(() => {}, [interestGroups, classMembers])
        ...
        <RelationGraph
          ...
          canvasPlugSlot={
            <>
                ...
                <div className="relative">
                  {interestGroups.map((group) => (
                    <div
                      key={group.groupId}
                      id={`location-${group.groupId}`}
                      className={`c-i-location ${
                        checkedGroupId.current === group.groupId ? 'c-i-location-active' : ''
                      }`}
                      style={{ left: group.location.x + 'px', top: group.location.y + 'px' }}
                ...
                <div>
                  {interestGroups.map((group) => (
                    <div
                      ...
                      id={`group-${group.groupId}`}
                      className={`c-i-group ${
                        checkedGroupId.current === group.groupId ? 'c-i-group-checked' : ''
                      }`}
                      onClick={() => onGroupClick(group.groupId)}
                    >
              ...
              <div className="w-72 absolute top-0 z-20" style={{ left: '800px' }}>
                <div>Class Members</div>
                <div>
                  {classMembers.map((member) => (
                    <div
                      key={member.name}
                      id={`member-${member.name}`}
                      className={`c-i-member ${
                        checkedMemberId.current === member.name ? 'c-i-member-checked' : ''
                      }`}
                      onClick={() => onMemberClick(member.name)}
                    >
                      <div className="c-i-name">{member.name}</div>
                      <div className="text-xs">Joined:</div>
                      <div className="c-i-count">{member.joinedGroups.length}</div>
                    </div>
                  ))}
                </div>
              </div>
