    ...
    <div style="height:calc(100vh);">
      <RelationGraph
        ...
        <template #graph-plug>
          <MyDemoPanel>
            <div class="c-content">
              点击选中单个节点后，点击节点四周的小按钮，就可以从对应的方向开始创建线条，同时可以自定义线条的形状、颜色、粗细等等属性。
            </div>
            <div class="c-content">
              同时，在选择线条终点时，还可以额外指定线条在终点的位置。
            </div>
          </MyDemoPanel>
          <RGEditingController>
...
<script>
import {RGEditingController, RGEditingConnectController, RGEditingNearNodeWidget, RGEditingCreateLineHandle} from 'relation-graph';
import MyDemoPanel from './rg-ui-simple/MyDemoPanel.vue';
export default {
  name: 'Demo',
  components: { MyDemoPanel, RGEditingController, RGEditingConnectController, RGEditingNearNodeWidget, RGEditingCreateLineHandle},
  data() {
    return {
...
import RelationGraph, { RGEditingController, RGEditingConnectController, RGEditingNearNodeWidget, RGEditingCreateLineHandle } from 'relation-graph-vue3';
...
const onNodeClick = (nodeObject: RGNode, $event: RGUserEvent) => {
    console.log('onNodeClick:', nodeObject);
    if ($event.shiftKey || $event.ctrlKey || ($event.metaKey && !$event.altKey)) {
        graphInstance.value.value.toggleEditingNode(nodeObject);
    } else {
...
export default GeeLineEdit2;
```

### `gee-line-edit-2.scss`
