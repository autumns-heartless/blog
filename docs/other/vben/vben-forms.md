...
思路：打开弹窗后调取后端接口拿到数据，分别给每个表单进行塞值（塞值我们怎么去塞呢，这里是通过拿到当前组件实例 ctx：当前组件； global：全局）去逐个进行赋值。在框架中使用 setFormData 这个方法去填值 ，使用 setFieldsValue 去设置表单字段 。 看一下具体代码！！！！
...
由于表单中有个别字段是需要提交给后端的，所以在提交方法里要设置提交字段的值，框架内是封装好的方法 getFieldsValue，看下面代码是如何实现的。
